...
function addSummarySheet(
  ...
  const metrics = [
    [copy.totalRevenue, data.stats.totalRevenue.value, copy.totalOrders, data.stats.totalOrders.value],
    [copy.cashRevenue, data.stats.cashRevenue.value, copy.averageOrder, data.stats.averageOrderValue.value],
...
function addDailySheet(
  ...
  const headers = [copy.date, copy.totalRevenue, copy.cashRevenue, copy.transferRevenue, copy.otherRevenue, copy.orderCount, copy.averageOrder];
  prepareTableSheet(sheet, copy.daily, headers, [14, 20, 20, 22, 24, 14, 20]);
  data.dailyRows.forEach((item) => {
    sheet.addRow([
      excelDate(item.date), item.totalRevenue, item.cashRevenue,
      item.transferRevenue, item.otherRevenue, item.orderCount,
  ...
  [2, 3, 4, 5, 7].forEach((column) => (sheet.getColumn(column).numFmt = MONEY_FORMAT));
...
function addGroupSheet(workbook: ExcelJS.Workbook, data: RevenueDashboardData, copy: (typeof COPY)["vi"] | (typeof COPY)["zh"]) {
  ...
  prepareTableSheet(sheet, copy.groups, [copy.group, copy.quantity, copy.revenue, copy.share], [36, 16, 22, 14]);
  const total = data.topProductGroups.reduce((sum, group) => sum + group.revenue, 0);
  data.topProductGroups.forEach((group) => sheet.addRow([group.groupName, group.quantity, group.revenue, total > 0 ? group.revenue / total : 0]));
  addTotalRow(sheet, 2, data.topProductGroups.length, copy.total, [2, 3]);
...
function addProductSheet(workbook: ExcelJS.Workbook, data: RevenueDashboardData, copy: (typeof COPY)["vi"] | (typeof COPY)["zh"]) {
  ...
  prepareTableSheet(sheet, copy.products, [copy.group, copy.product, copy.quantity, copy.revenue, copy.averagePrice, copy.share], [30, 42, 16, 22, 22, 14]);
  ...
  products.forEach((item) => sheet.addRow([item.groupName, item.name, item.quantity, item.revenue, item.quantity > 0 ? item.revenue / item.quantity : 0, total > 0 ? item.revenue / total : 0]));
  addTotalRow(sheet, 2, products.length, copy.total, [3, 4]);
  sheet.getColumn(3).numFmt = NUMBER_FORMAT;
  [4, 5].forEach((column) => (sheet.getColumn(column).numFmt = MONEY_FORMAT));
