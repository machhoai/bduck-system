...
const prodBase64 = envFile.match(/PROD_FIREBASE_SERVICE_ACCOUNT_BASE64="?(.*?)"?(\n|$)/)?.[1];
if (!prodBase64) throw new Error("Could not find PROD_FIREBASE_SERVICE_ACCOUNT_BASE64");

process.env.FIREBASE_SERVICE_ACCOUNT_BASE64 = prodBase64;
process.env.FIREBASE_PROJECT_ID = envFile.match(/PROD_FIREBASE_PROJECT_ID="?(.*?)"?(\n|$)/)?.[1] || "jw-system-f2104";
...
async function seed() {
  ...
  const { UserStatus } = await import('@bduck/shared-types');
  ...
  try {
    ...
    const user = await createUser(adminData, "SYSTEM_SEED");
    console.log("✅ Admin user created successfully!");
    console.log(`Email: ${adminData.email}`);
    console.log(`Password: ${adminData.password}`);
    console.log(`Username: ${adminData.username}`);
  } catch (error: any) {
    if (error.statusCode === 409 || error.code === 'auth/email-already-exists') {
      console.log("⚠️ Admin user already exists. If you forgot the password, you may need to reset it via Firebase Console or change the script to update the password.");
    } else {
