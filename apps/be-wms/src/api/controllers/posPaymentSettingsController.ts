...
import {
  ...
  posPaymentSettingsSchema,
  savePosPaymentSettings,
} from "../../services/posPaymentSettingsService.js";
...
const handleError = (response: Response, error: unknown) => {
  ...
  if (error instanceof z.ZodError)
    return sendError(
      response,
      { vi: "Cấu hình chuyển khoản không hợp lệ.", zh: "转账配置无效。" },
      400,
      error.flatten(),
    );
  ...
  return sendError(
    response,
    {
      vi: "Không thể xử lý cấu hình thanh toán POS.",
      zh: "无法处理 POS 支付配置。",
    },
    500,
  );
...
export const getPosPaymentSettingsHandler = async (
  request: Request,
  response: Response,
) => {
  try {
    const { deviceId } = posDeviceParamsSchema.parse(request.params);
    return sendSuccess(
      response,
      await getPosPaymentSettings(
        deviceId,
...
export const savePosPaymentSettingsHandler = async (
  request: Request,
  response: Response,
) => {
  try {
    ...
    return sendSuccess(
      ...
      {
        vi: "Đã cập nhật cấu hình thanh toán cho máy POS.",
        zh: "POS 设备支付配置已更新。",
      },
    );
  } catch (error) {
