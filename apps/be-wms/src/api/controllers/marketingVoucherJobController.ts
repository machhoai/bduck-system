...
const assertWorker = (req: Request, res: Response) => {
  ...
  if (!hasNonEmptySecret(secret)) {
    sendError(res, { vi: "Chưa cấu hình worker voucher.", zh: "尚未配置优惠券工作器。" }, 503);
    return false;
  }
  if (!securelyMatchesSecret(req.header("x-marketing-voucher-worker-secret"), secret)) {
    sendError(res, { vi: "Worker voucher không được xác thực.", zh: "优惠券工作器身份验证失败。" }, 401);
    return false;
  }
  return true;
};

export const listMarketingVoucherJobsHandler = async (req: Request, res: Response) => {
  try {
    const data = await getMarketingVoucherJobs(
      marketingVoucherJobQuerySchema.parse(req.query),
      requireRequestAuthorization(req),
    );
    return sendSuccess(res, data, { vi: "Đã tải danh sách job voucher.", zh: "优惠券任务列表已加载。" });
  } catch (error) {
    return handleMarketingVoucherError(res, error);
  }
};

export const getMarketingVoucherJobHandler = async (req: Request, res: Response) => {
  try {
    const { jobId } = marketingVoucherJobParamsSchema.parse(req.params);
    const data = await getMarketingVoucherJob(jobId, requireRequestAuthorization(req));
    return sendSuccess(res, data, { vi: "Đã tải job voucher.", zh: "优惠券任务已加载。" });
  } catch (error) {
    return handleMarketingVoucherError(res, error);
  }
};

export const resumeMarketingVoucherJobHandler = async (req: Request, res: Response) => {
  try {
    const { jobId } = marketingVoucherJobParamsSchema.parse(req.params);
    ...
    return sendSuccess(res, data, { vi: "Đã tiếp tục job voucher.", zh: "优惠券任务已恢复。" }, 202);
  } catch (error) {
    return handleMarketingVoucherError(res, error);
  }
};

export const processMarketingVoucherJobHandler = async (req: Request, res: Response) => {
  if (!assertWorker(req, res)) return;
  try {
    const { jobId } = marketingVoucherJobParamsSchema.parse(req.params);
    const data = await processMarketingVoucherJob(jobId);
    return sendSuccess(res, data, { vi: "Đã xử lý một chunk voucher.", zh: "优惠券任务分块已处理。" });
  } catch (error) {
    return handleMarketingVoucherError(res, error);
  }
};
