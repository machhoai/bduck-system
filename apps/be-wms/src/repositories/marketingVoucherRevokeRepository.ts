...
const nextCounts = (
  ...
  const distributed = codes.filter((code) => code.status === "DISTRIBUTED").length;
  if (
...
export const revokeMarketingVoucherCodesRecord = async (input: {
  ...
  const result = await db.runTransaction<MarketingVoucherRevokeResult & { replayed: boolean }>(
    async (transaction) => {
      ...
      if (operation.replay) {
        return { ...operation.replay, replayed: true } as MarketingVoucherRevokeResult & {
          replayed: boolean;
        };
      }
      const codeSnapshots = await transaction.getAll(...uniqueCodeIds.map(codeRef));
      const missing = codeSnapshots.filter((snapshot) => !snapshot.exists).map((snapshot) => snapshot.id);
      if (missing.length > 0) {
        throw marketingVoucherError(
          "MARKETING_VOUCHER_CODE_NOT_FOUND",
          { vi: "Không tìm thấy một hoặc nhiều mã voucher.", zh: "一个或多个优惠券码不存在。" },
          404,
          { code_ids: missing },
        );
      }
      ...
      if (used.length > 0) {
        throw marketingVoucherError(
          ...
          { vi: "Không thể vô hiệu mã đã sử dụng.", zh: "已使用的优惠券不能撤销。" },
          409,
          { code_ids: used.map((code) => code.id) },
        );
      }
      const changeable = codes.filter(
        (code) => !code.is_deleted && ["AVAILABLE", "DISTRIBUTED"].includes(code.status),
      );
      const campaignIds = [...new Set(changeable.map((code) => code.campaign_id))];
      ...
      const campaigns = campaignSnapshots.map((snapshot) => {
        if (!snapshot.exists || snapshot.get("is_deleted") === true) {
          throw marketingVoucherError(
            ...
            { vi: "Không tìm thấy chiến dịch của mã voucher.", zh: "未找到优惠券所属活动。" },
            404,
          );
        }
      ...
      campaigns.forEach((previous) => {
        const campaignCodes = changeable.filter((code) => code.campaign_id === previous.id);
