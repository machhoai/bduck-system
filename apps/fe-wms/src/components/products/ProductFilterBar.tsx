        ...
        <div className="space-y-3 rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-elevated)] p-4">
            ...
            <div className="grid grid-cols-2 gap-2 lg:grid-cols-7">
                ...
                <SelectFilter
                    ...
                    onChange={(value) =>
                        updateFilter("sortField", value as ProductFilters["sortField"])
                    }
                >
                    {Object.entries(t.products.sortFields).map(([key, label]) => (
                ...
                <SelectFilter
                    label={t.products.sortDirection}
                    value={filters.sortDirection}
                    onChange={(value) =>
                        updateFilter("sortDirection", value as ProductFilters["sortDirection"])
                    }
                >
                    <option value="asc">{t.auditLog.asc}</option>
                    <option value="desc">{t.auditLog.desc}</option>
                </SelectFilter>
            </div>
    ...
    return (
        <label className="min-w-0 space-y-1">
            ...
            <select
                ...
                className="h-8 w-full rounded-full border border-[var(--color-border-subtle)] bg-[var(--color-surface-input)] px-3 text-sm text-[var(--color-text-primary)] outline-none appearance-none transition-colors focus:border-[var(--color-border-focus)]"
            >
                {children}
            </select>
        </label>
