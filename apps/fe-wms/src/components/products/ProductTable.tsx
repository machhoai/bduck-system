...
import { Package, Search, Plus, Edit2, Trash2, Box, Image as ImageIcon } from 'lucide-react';
...
export function ProductTable({ products, loading, onEdit, onDelete, onAddNew }: ProductTableProps) {
                ...
                <td colSpan={6} className="px-6 py-16 text-center text-gray-500">
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <p className="text-sm text-gray-500 max-w-sm mx-auto">
                      {searchTerm ? 'Thử tìm kiếm với từ khóa khác.' : 'Chưa có dữ liệu sản phẩm trên hệ thống. Hãy thêm mới.'}
                    </p>
                  </div>
                </td>
              </tr>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-md bg-gray-100 border border-gray-200 overflow-hidden flex items-center justify-center flex-shrink-0">
                        {product.product_image_url && product.product_image_url.length > 0 ? (
                          <img src={product.product_image_url[0]} alt={product.name} className="w-full h-full object-cover" />
                        ) : (
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 line-clamp-1">{product.description || 'Không có mô tả'}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <p className="font-medium text-blue-700 bg-blue-50 px-2 py-0.5 rounded inline-block text-sm">{product.code}</p>
                    {product.barcode && (
                      <p className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                        <span className="w-3 h-3 inline-block bg-gray-200 rounded-sm"></span> {product.barcode}
                      </p>
                    )}
                  </td>
                  <td className="px-6 py-4">
                    ...
                    {product.product_origin && (
                      <p className="text-xs text-gray-500 mt-0.5">{product.product_origin}</p>
                    )}
                  </td>
