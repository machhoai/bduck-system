...
const TYPE_BADGE: Record<string, string> = {
    ...
    [ProductType.SOUVENIR_GIFT]: "bg-purple-100 text-purple-700 border-purple-200",
};
...
export function ProductCard(props: ProductCardProps) {
    ...
    const typeGradient = TYPE_GRADIENT[product.product_type] ?? "from-gray-50 to-gray-100";
    const typeBadge = TYPE_BADGE[product.product_type] ?? "bg-gray-100 text-gray-600 border-gray-200";
    const typeLabel =
        (t.inventoryDashboard.productTypes as Record<string, string>)[product.product_type]
    ...
    if (props.variant === "inventory") {
        ...
        return (
            <article className="group flex flex-col overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-elevated)] transition-shadow hover:shadow-md">
                {/* Image */}
                <div className={`relative h-32 w-full shrink-0 bg-gradient-to-br ${typeGradient}`}>
                    {primaryImage ? (
                        <Image src={primaryImage} alt={product.name} fill className="object-cover" sizes="(max-width: 640px) 50vw, 25vw" />
                    ) : (
                    ...
                    <div className={`absolute right-2 top-2 rounded-lg px-2 py-1 text-xs font-bold shadow ${isLowAtp ? "bg-red-500 text-white" : "bg-white/90 text-[var(--color-brand-primary)]"}`}>
                ...
                <div className="flex flex-1 flex-col gap-2 p-3">
                    <span className={`w-fit rounded-full border px-2 py-0.5 text-micro font-semibold ${typeBadge}`}>
                        {typeLabel}
                    </span>
                    <div>
                        <p className="line-clamp-2 text-sm font-semibold leading-tight text-[var(--color-text-primary)]">{product.name}</p>
                        <p className="mt-0.5 text-xxs text-[var(--color-text-muted)]">{product.code}</p>
                    </div>
                    <div className="mt-auto grid grid-cols-2 gap-1 rounded-lg bg-[var(--color-surface-card)] p-2">
                        <div>
                            ...
                            <div className="text-xs font-bold text-[var(--color-text-primary)]">{s.total.toLocaleString()}</div>
                        </div>
                        {s.onHold > 0 && (
                            <div>
                                <div className="text-xxs text-[var(--color-text-muted)]">{d.onHold}</div>
                                <div className="text-xs font-medium text-amber-600">{s.onHold.toLocaleString()}</div>
                            </div>
                        )}
                        {s.inTransit > 0 && (
                            <div>
                                <div className="text-xxs text-[var(--color-text-muted)]">{d.inTransit}</div>
                                <div className="text-xs font-medium text-blue-600">{s.inTransit.toLocaleString()}</div>
                            </div>
                        )}
                        <PriceCell price={product.unit_price ?? null} unit={product.unit} canViewPrice={canViewPrice} label={d.price} />
                    </div>
                </div>
            </article>
        );
