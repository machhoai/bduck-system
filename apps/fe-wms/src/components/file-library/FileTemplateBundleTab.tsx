...
import type { FileTemplate, FileTemplateBundle } from "@bduck/shared-types";
...
const text = {
  vi: {
    ...
    unavailable: "Một số biểu mẫu trong bộ đã bị xóa hoặc không còn khả dụng.",
  },
  zh: {
    create: "创建模板包",
    edit: "编辑模板包",
...
export default function FileTemplateBundleTab({
  ...
  const downloadBundle = async (bundle: FileTemplateBundle) => {
    const selected = bundle.template_ids
        ...
        (async () => {
          ...
          await Promise.all(
            selected.map(async (template, index) => {
              ...
              if (!response.ok) throw new Error(template.file_name);
              let name = safeFileName(template.file_name);
          ...
          {bundles.map((bundle) => {
            ...
            return (
              ...
              >
                ...
                <button
                  type="button"
                  disabled={!availableCount || downloadingId === bundle.id}
