...
interface FileLibraryToolbarProps {
    ...
    showTemplateCategoryFilter?: boolean;
}
...
const templateCategoryOptions: Array<FileTemplateCategory | "ALL"> = [
    "ALL",
...
export default function FileLibraryToolbar({
    ...
    showTemplateCategoryFilter = false,
                    ...
                    <div className="flex items-center gap-1 overflow-x-auto">
                        ...
                        {sourceOptions.map((option) => (
                            <button
                                ...
                                className={`h-7 shrink-0 rounded-[var(--radius-sm)] px-2 text-xs font-semibold transition ${filters.sourceType === option
                    ...
                    <div className="flex items-center gap-1 overflow-x-auto">
                        ...
                        {templateCategoryOptions.map((option) => (
                            <button
                                key={option}
                                type="button"
                                onClick={() => update("templateCategory", option)}
                                className={`h-7 shrink-0 rounded-[var(--radius-sm)] px-2 text-xs font-semibold transition ${filters.templateCategory === option
                                    ? "bg-[var(--color-brand-primary)] text-[var(--color-text-on-dark)]"
                                    : "bg-[var(--color-surface-subtle)] text-[var(--color-text-secondary)] hover:bg-[var(--color-brand-primary-muted)] hover:text-[var(--color-brand-primary)]"
                                    }`}
                            >
                    ...
                    {formatOptions.map((option) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex h-7 shrink-0 items-center gap-1 rounded-[var(--radius-sm)] px-2 text-xs font-semibold transition ${filters.format === option
