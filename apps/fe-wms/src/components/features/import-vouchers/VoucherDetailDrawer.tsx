...
function getCollectionName(entityType: string): string {
    switch (entityType) {
        case "EXPORT_VOUCHER":
            return "export_vouchers";
        case "TRANSFER_ORDER":
            return "transfer_orders";
        default:
            return "import_vouchers";
    }
}
...
export default function VoucherDetailDrawer({ voucher, onClose, onClone, onEdit }: VoucherDetailDrawerProps) {
    ...
    useEffect(() => {
        const itemsQuery = fsQuery(collection(db, collectionName, voucher.id, "items"));
                ...
                rawItems.map(async (item: any) => {
                    ...
                    if (item.product_id) {
                        try {
                            ...
                            if (pSnap.exists()) {
                                ...
                                productImageUrl =
                                    getFirstImageUrl(item.product_image_url) || getFirstImageUrl(p?.product_image_url);
                                unit = p?.unit || "";
                            }
                        } catch {
                            /* fallback to ID */
                    ...
                    if (item.warehouse_location_id) {
                        try {
                            ...
                            if (snap.exists())
                                warehouse_location_name = snap.data()?.name || item.warehouse_location_id;
                        } catch {}
                    }
                    ...
                    if (item.destination_location_id) {
                        try {
                            ...
                            if (snap.exists())
                                destination_location_name = snap.data()?.name || item.destination_location_id;
                        } catch {}
                    }
    ...
    const handleCreatorCancel = useCallback(
        async (otp?: string) => {
            ...
            if (!cancelReason.trim()) {
                gooeyToast.error(t.tasks.selfApproval.cancelReason, {
                    preset: "snappy",
                });
                return;
            }
    ...
    return (
        <>
            <div
                className="fixed inset-0 z-40 hidden bg-black/40 backdrop-blur-xs transition-opacity md:block"
                onClick={onClose}
            />
                    ...
                    <div className="flex-1 overflow-y-auto">
                        ...
                        <div className="px-4 pt-2">
                            ...
                            {entityType === "EXPORT_VOUCHER" ? (
                                <Field
                                    icon={User}
                                    label={t.tasks.detail.recipient || "Người nhận"}
                            ...
                            <Field
                                icon={User}
                                label={t.tasks.detail.creator}
                                value={creatorName || voucher.creator_id}
                            />
                            <Field
                                icon={Calendar}
                                label={t.tasks.detail.createdAt}
                            ...
                            {voucher.status === "COMPLETED" && (
                                <Field
                                    icon={CheckCircle2}
                                    label={(t as any).tasks?.detail?.completedAt || "Ngày hoàn thành"}
                            ...
                            {approvers.length > 0 ? (
                                ...
                                voucher.approver_id && (
                                    <Field
                                        ...
                                        label={(t as any).tasks?.detail?.approver || "Người duyệt"}
                                        value={legacyApproverName || voucher.approver_id}
                                    />
                                )
                            )}

                            {voucher.notes && (
                                <Field icon={FileText} label={t.tasks.detail.notes} value={voucher.notes} />
                            )}
                        </div>
