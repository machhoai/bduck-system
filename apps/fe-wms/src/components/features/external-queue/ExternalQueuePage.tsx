...
export default function ExternalQueuePage() {
    ...
    return (
        <div className="flex flex-col -mx-4 min-h-[calc(100dvh-80px)] bg-[var(--color-surface-subtle)] pb-24 sm:mx-0 sm:mt-0 sm:bg-transparent sm:pb-0">
                ...
                <div className="flex items-center h-full gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-sm leading-6 text-[var(--color-text-muted)]">
                            {externalQueueText?.subtitle ||
                                "Duyệt yêu cầu xuất kho từ thiết bị bên ngoài (như máy POS)"}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex h-full flex-1 flex-col gap-2">
                <div className="sticky top-[88px] z-20 lg:static">
                    <div className="flex items-center border-b border-[var(--color-border-subtle)] bg-white">
                        {visibleTabs.map((tab) => {
                            const Icon = tab.icon;
                            ...
                            return (
                                <button
                                    ...
                                    className={`group relative flex-1 flex items-center justify-center gap-2 px-4 py-2 text-sm font-semibold transition-colors ${isActive
                                        ? "text-[var(--color-brand-primary)]"
                                        : "text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)]"
                                        }`}
                                >
                                    <Icon
                                        className={`${isActive ? "text-[var(--color-brand-primary)]" : ""} size-6 md:size-4`
                                        }
                                    />
