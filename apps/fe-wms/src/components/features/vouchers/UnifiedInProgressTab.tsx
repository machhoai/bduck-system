...
const STATUS_CONFIG: Record<string, { bg: string; text: string }> = {
  ...
  APPROVED: {
    bg: "bg-[var(--color-status-approved-bg)]",
    text: "text-[var(--color-status-approved-text)]",
  },
  RECEIVING: {
...
export default function UnifiedInProgressTab({
  vouchers,
  initialTypeFilter,
  onClone,
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-3 rounded-[var(--radius-md)] border border-dashed border-[var(--color-border-subtle)] bg-[var(--color-surface-elevated)] py-20 text-center">
        ...
        <p className="text-sm font-semibold text-[var(--color-text-secondary)]">
          {(t as any).vouchers?.inProgressTab?.emptyTitle ||
            "Chưa có lệnh nào đang xử lý"}
        </p>
        <p className="w-full text-xs leading-5 text-[var(--color-text-muted)]">
          {(t as any).vouchers?.inProgressTab?.emptyHint ||
            "Lệnh nháp hoặc đang chờ duyệt sẽ xuất hiện ở đây."}
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex gap-2">
          ...
          >
            {(t as any).vouchers?.inProgressTab?.filterImport ||
              misc.importWarehouse}
          </button>
          <button
            onClick={() =>
          ...
          >
            {(t as any).vouchers?.inProgressTab?.filterExport ||
              misc.exportWarehouse}
          </button>
          <button
            onClick={() =>
        ...
        >
          ...
          {statusOptions.map((status) => (
            <option key={status} value={status}>
              {(t as any).importVoucher?.status?.[status] ||
                (t as any).exportVoucher?.status?.[status] ||
                status}
            </option>
          ))}
        </select>
        ...
        <select
          ...
          className="h-8 max-w-[150px] rounded-[var(--radius-sm)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-input)] px-2 text-xs outline-none focus:border-[var(--color-border-focus)]"
        >
          <option value="">Người duyệt (Tất cả)</option>
          {users.map((user) => (
            <option key={user.id} value={user.id}>
              {user.full_name || user.email}
            </option>
