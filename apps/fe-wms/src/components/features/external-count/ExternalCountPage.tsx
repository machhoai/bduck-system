...
import { CheckCircle2, ClipboardList, Search, Settings2 } from "lucide-react";
...
import { FilterSelect, Metric, ToggleRow } from "./ExternalCountPageControls";
...
export default function ExternalCountPage() {
  ...
  const [config, setConfig] = useState<ExternalCountRequirementConfig | null>(
    null,
  );
  ...
  const visibleSessions = useMemo(() => {
    ...
    return [...sessions]
      ...
      .filter(
        (session) =>
          !locationId || session.warehouse_location_id === locationId,
      )
      .filter(
        (session) => !businessDate || session.business_date === businessDate,
      )
      .filter((session) => {
        if (!q) return true;
