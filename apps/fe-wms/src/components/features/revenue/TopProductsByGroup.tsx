  ...
  return (
    <section className="rounded-[var(--radius-lg)] bg-[var(--color-surface-elevated)] p-4">
      <div className="flex flex-col gap-0.5">
        <h2 className="text-sm font-bold text-[var(--color-text-primary)]">{d.topProducts.title}</h2>
            ...
            <div key={group.groupName} className="rounded-[var(--radius-sm)] bg-[var(--color-surface-card)] p-3">
              <div className="flex items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-2">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[var(--color-brand-primary)] text-xxs font-bold text-white">
                    {index + 1}
                  </span>
                  <div className="min-w-0">
                    <h3 className="truncate text-sm font-bold text-[var(--color-text-primary)]">{group.groupName}</h3>
                    <p className="text-xxs text-[var(--color-text-muted)]">
                      {formatNumber(group.quantity)} {d.topProducts.units}
                    </p>
                  </div>
                </div>
                <span className="shrink-0 text-sm font-extrabold tabular-nums text-[var(--color-text-primary)]">
              ...
              <div className="mt-2.5 flex flex-col gap-1">
                {group.items.map((item) => (
                  <div key={item.name} className="flex items-center justify-between gap-3 px-2 py-1">
                    <div className="min-w-0">
                      <p className="truncate text-xs font-semibold text-[var(--color-text-primary)]">{item.name}</p>
                      <p className="text-xxs text-[var(--color-text-muted)]">
