...
export function resolveAttendanceDay({
  ...
  if (isHoliday && !hasCheckIn) {
    return { ...emptyResolution(), status: "CL", holidayUnits: 1 };
  }
...
export function buildTimeAttendanceWorkbook({
  ...
  const sheet = workbook.addWorksheet("Chấm công", {
    ...
    views: [{ state: "frozen", xSplit: 4, ySplit: 14, showGridLines: false }],
    pageSetup: {
  ...
  sheet.getCell(5, 1).value = "BẢNG CHẤM CÔNG";
  sheet.mergeCells(6, 1, 6, lastColumn);
  sheet.getCell(6, 1).value = periodLabel(days);

  const legendWidth = Math.floor(lastColumn / 3);
  const legendRanges: Array<[number, number, string]> = [
    [
      1,
      legendWidth,
      "Ký hiệu trạng thái\nx: Có check-in   P: Nghỉ phép cả ngày   N0: Nghỉ không lương   KP: Nghỉ không phép\nsP/cP: Nghỉ phép nửa ngày và có check-in nửa ngày còn lại   CL: Công ty cho nghỉ hưởng lương",
    ],
    [
      legendWidth + 1,
      legendWidth * 2,
      "Xử lý nghỉ nửa ngày\nsP/KP: Nghỉ phép sáng, chiều không check-in\nKP/cP: Sáng không check-in, nghỉ phép chiều\nĐơn đang chờ duyệt vẫn dùng mã nghỉ tương ứng",
    ],
    [
      legendWidth * 2 + 1,
      lastColumn,
      "Hiển thị và tổng hợp\nx dùng nền xanh; mã đặc biệt chỉ đổi màu chữ. Check-in sau 08:45 tô vàng, không áp dụng cho sP.\n(1) Công thực tế   (2) Phép có lương   (3) KP   (4) N0   (5) CL   (6) Công tính lương = (1) + (2) + (5)",
    ],
  ];
  legendRanges.forEach(([from, to, text]) => {
    sheet.mergeCells(8, from, 10, to);
    const cell = sheet.getCell(8, from);
    cell.value = text;
  ...
  fixedHeaders.forEach((header, index) => {
    ...
    sheet.mergeCells(12, column, 14, column);
    sheet.getCell(12, column).value = header;
  });
  sheet.mergeCells(12, firstDayColumn, 12, lastDayColumn);
  sheet.getCell(12, firstDayColumn).value = "Ngày trong kỳ";
  days.forEach((day, index) => {
    const column = firstDayColumn + index;
    sheet.getCell(13, column).value = day.weekday;
    sheet.getCell(14, column).value = Number(day.label);
  });
  ["(1)", "(2)", "(3)", "(4)", "(5)", "(6)"].forEach(
    (header, index) => {
  ...
  for (let row = 12; row <= 14; row += 1) {
    for (let column = 1; column <= lastColumn; column += 1) {
      ...
      cell.fill = solidFill(row === 12 ? "FF1F4E78" : "FF4472C4");
  ...
  rows.forEach((employee, employeeIndex) => {
    const statusRow = 15 + employeeIndex * 3;
  ...
  const totalRow = 15 + rows.length * 3;
  ...
  sheet.getCell(5, 1).font = { name: "Arial", size: 16, bold: true, color: { argb: "FF17365D" } };
  sheet.getCell(5, 1).alignment = { horizontal: "center", vertical: "middle" };
  sheet.getCell(6, 1).font = { name: "Arial", size: 11, italic: true, color: { argb: "FF475569" } };
  sheet.getCell(6, 1).alignment = { horizontal: "center", vertical: "middle" };
  sheet.getRow(5).height = 28;
  sheet.getRow(6).height = 20;
  [8, 9, 10].forEach((row) => (sheet.getRow(row).height = 34));
