import type { LeaveBalanceSummary } from "@bduck/shared-types";
import { ListRestart, Plus, ShieldCheck } from "lucide-react";

import { MetricTile } from "./AdminOverviewParts";

const formatUnits = (value: number | undefined) =>
  (value ?? 0).toLocaleString("vi-VN", { maximumFractionDigits: 1 });

export function LeaveBalanceCard({
  labels,
  summary,
  loading,
  error,
  canView,
  canRequest,
  onHistory,
  onUsedLeave,
  onRequest,
}: {
  labels: Record<string, string>;
  summary: LeaveBalanceSummary | null;
  loading: boolean;
  error: string | null;
  canView: boolean;
  canRequest: boolean;
  onHistory: () => void;
  onUsedLeave: () => void;
  onRequest: () => void;
}) {
  const currentYear = Number(
    summary?.as_of_date?.slice(0, 4) || new Date().getFullYear(),
  );
  const carryover = summary?.buckets.find(
    (bucket) => bucket.leave_year === currentYear - 1,
  );
  return (
    <section
      data-employee-admin-animate
      className="rounded-2xl border border-[var(--color-border-subtle)] bg-white p-4 shadow-xs lg:p-5"
    >
      {/* Standardized Header */}
      <div className="flex items-center justify-between border-b border-[var(--color-border-soft)] pb-3">
        <div className="flex items-center gap-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#257a3e10] text-[#257a3e]">
            <ShieldCheck size={15} />
          </div>
          <div>
            <h3 className="text-sm font-bold tracking-wider text-[var(--color-text-primary)]">
              {labels.leaveBalanceTitle || labels.leaveBalance}
            </h3>
          </div>
        </div>
      </div>

      {loading ? (
        <div className="mt-4 grid grid-cols-3 gap-2">
          {[0, 1, 2].map((item) => (
            <div
              key={item}
              className="h-16 animate-pulse rounded-xl bg-[var(--color-surface-card)]"
            />
          ))}
        </div>
      ) : canView && summary ? (
        <>
          <div className="mt-3.5 grid grid-cols-3 gap-2">
            <MetricTile
              label={labels.availableLeave}
              value={formatUnits(summary.available_units)}
              tone="success"
            />
            <MetricTile
              label={labels.pendingLeave}
              value={formatUnits(summary.held_units)}
            />
            <MetricTile
              label={labels.usedLeave}
              value={formatUnits(summary.used_units)}
              tone="warning"
              actionLabel={labels.viewRequestHistory}
              onClick={onUsedLeave}
            />
          </div>
          <div className="mt-3 flex items-center justify-between gap-3 rounded-xl bg-[var(--color-surface-card)] px-3 py-2">
            <span className="text-xs text-[var(--color-text-muted)]">
              {labels.probationLeave}
            </span>
            <span className="text-xs font-bold text-[var(--color-text-primary)]">
              {formatUnits(summary.pending_probation_units)}
            </span>
          </div>
          {carryover && carryover.available_units > 0 && (
            <div className="mt-2 flex items-center justify-between gap-3 rounded-xl bg-amber-50 px-3 py-2 border border-amber-200/60">
              <span className="text-xs font-medium text-amber-800">
                {labels.carryoverLeaveExpiry.replace(
                  "{date}",
                  `31/03/${currentYear}`,
                )}
              </span>
              <span className="text-xs font-bold text-amber-900">
                {formatUnits(carryover.available_units)}
              </span>
            </div>
          )}
        </>
      ) : (
        <p className="mt-3.5 rounded-xl bg-[var(--color-surface-card)] px-3 py-3 text-xs text-[var(--color-text-muted)]">
          {error || labels.leaveBalanceNoAccess}
        </p>
      )}

      <p className="mt-2.5 text-xs leading-relaxed text-[var(--color-text-muted)]">
        {labels.leaveBalanceHint}
      </p>

      <div className="mt-3.5 flex flex-wrap items-center gap-2">
        {canView && (
          <button
            type="button"
            onClick={onHistory}
            className="inline-flex h-8 flex-1 items-center justify-center gap-1.5 rounded-xl border border-[var(--color-border-subtle)] bg-white px-3 text-xs font-semibold text-[var(--color-text-primary)] transition-all hover:bg-[var(--color-surface-card)] active:scale-95 cursor-pointer shadow-2xs"
          >
            <ListRestart size={14} />
            <span>{labels.viewLeaveHistory}</span>
          </button>
        )}

        {canRequest && (
          <button
            type="button"
            onClick={onRequest}
            className="inline-flex h-8 flex-1 items-center justify-center gap-1.5 rounded-xl bg-[var(--color-brand-primary)] px-3 text-xs font-semibold text-white shadow-2xs transition-all hover:bg-[var(--color-brand-primary-hover)] active:scale-95 cursor-pointer"
          >
            <Plus size={14} />
            <span>{labels.createRequest}</span>
          </button>
        )}
      </div>
    </section>
  );
}
