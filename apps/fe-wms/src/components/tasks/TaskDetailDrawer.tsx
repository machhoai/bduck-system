...
export default function TaskDetailDrawer({ approval, isSelfCreated, onClose }: TaskDetailDrawerProps) {
    ...
    const handleForceCancel = useCallback(
        async (otp?: string) => {
            ...
            const submitAction = async () => {
                return forceCancelApproval(approval.entity_type, approval.entity_id, cancelReason, otp);
            };
                                            ...
                                            {items.map((item, idx) => (
                                                ...
                                                >
                                                    <div className="flex items-start justify-between gap-2">
                                                        ...
                                                        <div className="text-right flex-shrink-0">
                                                            <p className="text-sm font-semibold text-[var(--color-text-primary)]">
                                                                {formatCurrency(
                                                                    item.expected_quantity * item.unit_price,
                                                                )}
                                                                đ
                                                            </p>
                                                        </div>
                                                    ...
                                                    <div className="mt-1.5 text-xs text-[var(--color-text-muted)]">
                                                        {t.tasks.detail.quantity}:{" "}
                                                        {formatCurrency(item.expected_quantity)}
                                                        {item.unit_price > 0 && (
