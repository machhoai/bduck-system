      ...
      {isLoading ? (
        ...
        <section className="space-y-3">
          ...
          <div
            ...
            className="relative h-96 overflow-auto rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)] bg-white"
          >
            {roles.map((role) => {
...
function ModeButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`h-8 rounded-full px-4 text-sm transition-all active:scale-95 ${active
...
function RoleCardContent({
  ...
  return (
    <div className="flex min-w-0 items-center gap-3">
      <span
        className="flex h-8 w-10 shrink-0 items-center justify-center rounded-full text-white"
    ...
    <div className="rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)] bg-white p-4">
      {Array.from({ length: 5 }).map((_, index) => (
        <div key={index} className="flex items-center gap-3 py-3">
          <Skeleton variant="circle" className="h-8 w-10" />
