...
const PERMISSION_PAGE_DEFINITIONS: PermissionPageDefinition[] = [
  ...
  {
    id: "stock-counts",
    label: { vi: "Trang Kiểm đếm kho", zh: "库存盘点页面" },
    description: {
      vi: "Tạo phiên, nhập kết quả và xử lý phiên kiểm đếm nội bộ theo phạm vi kho.",
      zh: "按仓库授权范围创建、录入和处理内部库存盘点会话。",
    },
    sections: [
      {
        id: "stock-count-session",
        label: { vi: "Phiên kiểm đếm nội bộ", zh: "内部库存盘点会话" },
        description: {
          vi: "Tách riêng quyền xem, tạo phiên, nhập kết quả và hủy phiên kiểm đếm.",
          zh: "分别控制查看、创建、录入结果和取消盘点会话。",
        },
        keys: [
          "stock_counts.view",
          "stock_counts.create",
        ...
        const sections = page.sections
          .map((section) => {
            const permissions = section.keys
              ...
              .filter((permission): permission is PermissionDefinition =>
                Boolean(permission),
              );
    ...
    const fallbackSections = [...PERMISSION_GROUPS]
      ...
      .map((group) => ({
        ...
        permissions: PERMISSION_REGISTRY.filter(
          (permission) =>
            permission.group === group.id && !assignedKeys.has(permission.key),
        ),
      }))
      .filter((section) => section.permissions.length > 0);
