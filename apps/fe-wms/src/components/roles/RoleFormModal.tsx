...
/** Group permissions by their group id, sorted by group order */
function useGroupedPermissions(lang: "vi" | "zh") {
    return useMemo(() => {
        const sortedGroups = [...PERMISSION_GROUPS].sort(
            (a, b) => a.order - b.order,
        );
        const permsByGroup = new Map<string, PermissionDefinition[]>();
        for (const perm of PERMISSION_REGISTRY) {
            const list = permsByGroup.get(perm.group) || [];
            list.push(perm);
            permsByGroup.set(perm.group, list);
        }

        return sortedGroups
            .filter((g) => permsByGroup.has(g.id))
            .map((group) => ({
                group,
                permissions: permsByGroup.get(group.id)!,
...
export function RoleFormModal({
    ...
    useEffect(() => {
        ...
        // Expand all groups that have active permissions
        const activeGroups = new Set<string>();
        if (role?.permissions) {
            for (const perm of PERMISSION_REGISTRY) {
                if (role.permissions[perm.key]) activeGroups.add(perm.group);
            }
        }
                ...
                >
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {groupedPermissions.map(({ group, permissions: groupPerms, label: groupLabel }) => {
                                const isExpanded = expandedGroups.has(group.id);
                                const activeCount = groupPerms.filter(
                                        ...
                                        {isExpanded && (
                                            <div className="border-t border-[var(--color-border-soft)] bg-white">
                                                ...
                                                {groupPerms.map((perm) => (
                                                    <label
                                                        key={perm.key}
                                                        className="flex cursor-pointer items-start gap-3 px-4 py-3 transition-colors hover:bg-[var(--color-surface-card)]"
                                                    >
                                                        <input
                                                            type="checkbox"
