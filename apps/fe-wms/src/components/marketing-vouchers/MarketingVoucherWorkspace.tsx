...
export function MarketingVoucherWorkspace() {
    ...
    return (
        <main className="min-h-full flex flex-col gap-3">
            <div className="w-full flex-1">
                <header className="rounded-xl border border-amber-100 bg-gradient-to-br from-amber-50 via-white to-white p-3 sm:p-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <div className="flex items-center gap-3">
                                <span className="grid h-8 w-8 place-items-center rounded-lg bg-amber-500 text-slate-950 shadow-sm">
                        ...
                        <div
                            className={`inline-flex w-fit items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${fromCache ? "bg-amber-100 text-amber-800" : "bg-emerald-50 text-emerald-700"}`}
                        >
                            <span className="relative flex h-1.5 w-1.5">
                                ...
                                />
                                <span
                                    className={`relative inline-flex h-1.5 w-1.5 rounded-full ${fromCache ? "bg-amber-600" : "bg-emerald-600"}`}
                                />
                            </span>
                ...
                <nav
                    ...
                    className="sticky top-0 z-20 mt-3 overflow-x-auto border-y border-slate-100 bg-white/95 backdrop-blur sm:rounded-lg sm:border"
                >
                    <div className="flex min-w-max gap-1 p-1">
                        {tabs.map(({ id, label, icon: Icon }) => (
                            <button
                                ...
                                className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition ${activeTab === id ? "bg-slate-950 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100 hover:text-slate-950"}`}
                            >
                                <Icon size={14} />
                                {label}
                            </button>
                ...
                <section className="mt-3">
                    {hasError ? (
                        <div
                            role="alert"
                            className="mb-3 flex items-center gap-1.5 rounded-lg bg-rose-50 px-3 py-2 text-sm font-medium text-rose-700"
                        >
                            <Activity size={14} />
                            {copy.loadError}
                        </div>
