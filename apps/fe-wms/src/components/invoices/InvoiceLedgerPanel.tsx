...
const saveInvoiceData = (
  result: Awaited<ReturnType<typeof invoiceApi.downloadPublishedInvoice>>,
) => {
  if (result.isUrl) {
  ...
  const mime =
    result.type === "Pdf" ? "application/pdf" : "application/xml;charset=utf-8";
  let blob: Blob;
...
export function InvoiceLedgerPanel({
  ...
  const handleView = async (item: InvoiceLedgerEntryView) => {
    ...
    setError(null);
    try {
      const result = await invoiceApi.viewPublishedInvoice(
        item.id,
        warehouseId,
      );
  ...
  const handleDownload = async (
    ...
    try {
      saveInvoiceData(
        await invoiceApi.downloadPublishedInvoice(item.id, warehouseId, type),
      );
    } catch (nextError) {
      const presented = invoiceErrorToast(
        nextError,
  ...
  const submitResolveCase = async () => {
    ...
    try {
      ...
      const presented = invoiceErrorToast(
        nextError,
        "SAVE",
        "Không thể đóng sai lệch đối chiếu.",
      );
      setError(presented.description);
      showToast.error(presented.title, presented.description);
    } finally {
          ...
          <div className="flex items-center justify-between border-b border-slate-200 p-2.5">
            <div>
              ...
              <p className="text-xxs text-slate-500">
                JoyWorld ↔ MISA meInvoice · {businessDate}
              </p>
            </div>
            <button
              type="button"
              onClick={() => void load()}
              disabled={loading}
              className="inline-flex h-8 items-center gap-1.5 rounded-md border border-slate-200 px-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50"
            >
          ...
          {loading ? (
            <div className="p-3 space-y-2 animate-pulse" aria-label="Đang tải…">
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className="flex h-10 items-center gap-4 rounded-md bg-slate-50 px-3"
                >
            ...
            <div className="flex min-h-48 items-center justify-center text-xs text-slate-500">
              Chưa có dữ liệu phù hợp. Hãy đồng bộ ngày đã chọn.
            </div>
          ) : (
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-center justify-between">
                      ...
                      <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-600">
                        <div>
                          ...
                          <span
                            ...
                            title={item.customer_name || "Khách lẻ"}
                          >
                    ...
                    <div className="flex flex-wrap items-center justify-between md:justify-end gap-3 shrink-0 pt-2 md:pt-0 border-t md:border-t-0 border-slate-100">
                      <div className="flex flex-col text-right">
                        <span className="text-[11px] text-indigo-800 uppercase tracking-wider font-bold">
                          Tổng tiền
                        </span>
                        ...
                        <span
                          className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${
                            ...
                            InvoiceOrderMatchStatus.MATCHED
                              ...
                              : item.match_status ===
                                  InvoiceOrderMatchStatus.NOT_ISSUED
        ...
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          ...
          <div className="divide-y divide-slate-100">
            {openCases.map((item) => (
              <div
                key={item.id}
                className="flex flex-col gap-1.5 p-3 sm:flex-row sm:items-center text-xs"
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-xs text-slate-500">
                    Đơn: {item.source_order_document_id ?? "không xác định"} ·
                    MISA:{" "}
                  ...
                  {item.resolution_note && (
                    <div className="mt-0.5 text-xs text-emerald-700">
                      Kết quả: {item.resolution_note}
                    </div>
                  )}
                </div>
                <span
                  className={`rounded-full px-2.5 py-1 text-xs font-semibold ${item.status === InvoiceReconciliationCaseStatus.OPEN ? "bg-rose-50 text-rose-700 border border-rose-200" : "bg-emerald-50 text-emerald-700 border border-emerald-200"}`}
                >
                  {item.status}
                </span>
                {item.status === InvoiceReconciliationCaseStatus.OPEN && (
                  <button
                    type="button"
                    disabled={!canResolve || workingId === item.id}
                    onClick={() => resolveCase(item)}
                    className="h-7 rounded-lg border border-slate-200 px-2.5 text-xs font-semibold disabled:opacity-50 hover:bg-slate-50"
                  >
                    Đánh dấu đã xử lý
                  </button>
                )}
...
function ActionButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className="inline-flex h-7 items-center gap-1 rounded-lg border border-slate-200 px-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-100 disabled:opacity-40 transition-colors"
    >
