          ...
          <div className="mt-2 flex flex-wrap gap-x-3 gap-y-0.5 text-xxs text-slate-600">
            ...
            {progress.pendingWithMisaIdentity > 0 && (
              <span className="font-semibold text-amber-700">
                {progress.pendingWithMisaIdentity}{" "}
                {d.statusMisaReturnedIdentity}
              </span>
            )}
            {progress.pendingWithoutMisaIdentity > 0 && (
              <span className="font-semibold text-rose-700">
                {progress.pendingWithoutMisaIdentity}{" "}
                {d.statusPendingWithoutIdentity}
              </span>
            )}
            {progress.retrying > 0 && (
              <span>
              ...
              <div className="mt-2 flex flex-wrap gap-1">
                {retryCandidates.slice(0, 6).map((item) => (
                  <span
                    ...
                    title={item.message}
                  >
                    {item.order_number ?? item.item_id} ·{" "}
                    {item.misa_error_code ?? item.status}
                  </span>
                ))}
