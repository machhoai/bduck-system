...
import { EmployeeDetailAttendanceTab } from "./EmployeeDetailAttendanceTab";
import { EmployeeDetailLeaveTab } from "./EmployeeDetailLeaveTab";
import {
    getVisibleEmployeeDetailTabs,
    type EmployeeDetailTabKey,
} from "./employeeDetailTabPolicy";
import { EmployeeDetailTabs } from "./EmployeeDetailTabs";
import { employeeDetailTabsTranslations } from "./employeeDetailTabsTranslations";
...
export function EmployeeDetailBottomSheet({
    ...
    const [activeTab, setActiveTab] =
        useState<EmployeeDetailTabKey>("profile");
    ...
    const canReadOwnLeave =
        ...
        isSelf &&
        hasPermission("leave.self.read", facilityId);
    ...
    const canViewAttendance =
        Boolean(profile) &&
        hasPermission("attendance.view", facilityId);
    const detailTabLabels = employeeDetailTabsTranslations[lang];
        ...
        <div className="flex flex-col gap-3.5">
            ...
            <div className="space-y-2 rounded-2xl border border-[var(--color-border-subtle)] bg-white p-3.5 shadow-2xs">
                ...
                <div className="grid grid-cols-1 gap-2.5 pt-1">
                    <div className="flex items-center gap-2 rounded-xl bg-[#f8fafc] p-2.5">
                        <Mail
                            size={14}
            ...
            <div className="space-y-2 rounded-2xl border border-[var(--color-border-subtle)] bg-white p-3.5 shadow-2xs">
                <div className="flex items-center justify-between border-b border-[var(--color-border-soft)] pb-2">
                    ...
                    {user ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 border border-emerald-200 px-2 py-0.5 text-xs font-semibold text-[#257a3e]">
                            <UserCheck size={12} />
                            {details.linkedStatus}
                        </span>
                    ) : (
                        <span className="inline-flex items-center gap-1 rounded-full bg-slate-50 border border-slate-200 px-2 py-0.5 text-xs font-semibold text-[var(--color-text-muted)]">
                            <UserX size={12} />
    ...
    const employmentInfoContent = (
        <div className="flex flex-col gap-3.5">
            {/* Thông tin trạng thái lao động */}
            <div className="space-y-2 rounded-2xl border border-[var(--color-border-subtle)] bg-white p-3.5 shadow-2xs">
                <div className="flex items-center justify-between border-b border-[var(--color-border-soft)] pb-2">
                    <div className="flex items-center gap-2">
                        <Calendar size={15} className="text-[var(--color-brand-primary)]" />
                    ...
                    <span className="inline-flex rounded-full bg-blue-50 border border-blue-200/80 px-2.5 py-0.5 text-xxs font-bold text-blue-700">
                        {
    ...
    const activeTabContent =
        activeTab === "profile" ? (
            <div
                aria-labelledby="employee-detail-tab-profile"
                className="grid grid-cols-1 items-start gap-5 lg:grid-cols-12"
                id="employee-detail-panel-profile"
                role="tabpanel"
            >
                <div className="lg:col-span-7">{leftColumnContent}</div>
                <div className="lg:col-span-5">{employmentInfoContent}</div>
            </div>
        ) : activeTab === "contracts" ? (
            <div
                aria-labelledby="employee-detail-tab-contracts"
                id="employee-detail-panel-contracts"
                role="tabpanel"
            >
                {contractContent}
            </div>
    ...
    if (isMobile) {
        return (
            ...
            >
                <EmployeeDetailTabs
                    activeTab={activeTab}
                    labels={detailTabLabels.tabs}
                    onChange={setActiveTab}
                    tabs={visibleTabs}
                />
                <div className="pt-2 text-sm text-[var(--color-text-primary)]">
                    {activeTabContent}
                </div>
            </BottomSheet>
        );
    ...
    return (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            <div
                ...
                onClick={onClose}
            />
            <div className="relative z-50 flex max-h-[90vh] min-h-[90vh] w-[94vw] max-w-5xl xl:max-w-6xl flex-col overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-elevated)] shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                {/* Fixed Header */}
                <div className="flex items-center justify-between border-b border-[var(--color-border-soft)] px-5 py-3.5 bg-white/80 backdrop-blur-md shrink-0">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            ...
                            <p className="text-xxs text-[var(--color-text-muted)] font-mono">
                    ...
                    <button
                        type="button"
                        onClick={onClose}
                        className="rounded-full p-2 text-[var(--color-text-muted)] transition-all hover:bg-[var(--color-surface-card)] hover:text-[var(--color-text-primary)] active:scale-95 cursor-pointer"
                ...
                <EmployeeDetailTabs
                    ...
                    tabs={visibleTabs}
                />
                <div className="flex-1 overflow-y-auto p-5 bg-[#f8fafc]/50">
                    {activeTabContent}
                </div>
            </div>
        </div>
...
function StatusBadge({
    ...
    const tone =
        status === EmployeeProfileStatus.ACTIVE
            ? "text-[var(--color-accent-success)] bg-emerald-50"
            : status === EmployeeProfileStatus.ON_LEAVE
                ? "text-[var(--color-accent-warning)] bg-amber-50"
                : "text-[var(--color-text-muted)] bg-slate-100";
    return (
        <span
            className={`inline-flex rounded-full px-2 py-0.5 text-xxs font-semibold ${tone}`}
        >
