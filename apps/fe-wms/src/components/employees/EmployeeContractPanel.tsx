...
import { executeEmployeeContractSave } from "./employeeContractSaveFlow";
...
export function EmployeeContractPanel({
    ...
    const saveContract = async (values: EmployeeContractDraftValues) => {
        ...
        const mode = currentForm?.mode ?? "create";
        try {
            const result = await showToast.promise(
                executeEmployeeContractSave({
                    profileId: profile.id,
                    mode,
                    sourceContract: currentForm?.contract ?? null,
                    values,
                    labels,
                }),
                {
                    loading:
                        mode === "renew" ? labels.toasts.renewing : labels.toasts.saving,
                    success:
                        mode === "renew" ? labels.toasts.renewed : labels.toasts.saved,
            ...
            emitDataMutation(["employee_contracts", "employee_contract_documents"]);
            setSelectedId(result.contract.id);
            setForm(null);
        } catch (caught) {
    ...
    return (
        <section className="space-y-4 rounded-2xl border border-[var(--color-border-subtle)] bg-white p-3.5 shadow-2xs">
                    ...
                    <>
                        {selected ? (
                            <EmployeeContractActionBar
                                ...
                                onLifecycle={(mode) =>
                                    setLifecycle({ mode, contract: selected })
                                }
                            />
                        ) : null}
            ...
            />
            <div className="flex md:hidden w-full flex-wrap justify-end gap-2">
                {canImportHistory ? (
                    <button
                        type="button"
                        onClick={() => setImportOpen(true)}
                        className="inline-flex flex-1 justify-center h-8 shrink-0 items-center gap-1.5 rounded-full border border-blue-200 px-3 text-xs font-semibold text-blue-700"
                    >
                        <Sheet size={13} />
                        {importLabels.open}
                    </button>
                ...
                {canManage ? (
                    <button
                        type="button"
                        onClick={() => setForm({ mode: "create", contract: null })}
                        className="inline-flex flex-1 justify-center h-8 shrink-0 items-center gap-1.5 rounded-full bg-[var(--color-brand-primary)] px-3 text-xs font-semibold text-white"
                    >
                        <FilePlus2 size={13} />
                        {labels.create}
                    </button>
