...
export function PosSettingsPanel({ warehouseId, storeName, settings, canManage, onChanged }: {
    ...
    const [draftForm, setDraftForm] = useState<PosReceiptSettingsPayload | null>(null);
    ...
    const update = useCallback(<K extends keyof PosReceiptSettingsPayload>(key: K, value: PosReceiptSettingsPayload[K]) => {
        setDraftForm((current) => ({ ...(current ?? synchronizedForm), [key]: value }));
    }, [synchronizedForm]);
    ...
    const save = useCallback(async () => {
        ...
        const action = async () => {
            try {
                const saved = await posManagementApi.saveReceiptSettings(warehouseId, form);
    ...
    const handleLogoFile = useCallback((file: File) => {
        ...
        reader.addEventListener("load", () => {
            ...
            setDraftForm((current) => ({ ...(current ?? synchronizedForm), logo_data_url: reader.result as string, show_logo: true }));
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h2 className="text-sm font-bold text-slate-900">{managementCopy.receiptTitle}</h2>
                    <p className="text-xs text-slate-500">{managementCopy.receiptHint} · {managementCopy.version} {settings?.version ?? 0}</p>
                </div>
                {canManage && (
                    <div className="flex items-center gap-2">
                        <button type="button" onClick={() => setDraftForm(createDefaultPosReceiptSettings(storeName))} disabled={saving} className="flex h-9 items-center gap-2 rounded-lg border border-slate-200 px-3 text-xs font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-50"><RotateCcw size={14} /> {copy.reset}</button>
