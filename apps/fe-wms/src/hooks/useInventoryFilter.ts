...
export function useInventoryFilter(
    ...
    const filteredRows = useMemo<InventoryRow[]>(() => {
        ...
        let rows = aggregated.filter((row) => {
            ...
            // Product type filter
            if (filters.productType && row.product.product_type !== filters.productType) {
                return false;
            }

            // Product origin filter
            if (filters.productOrigin && row.product.product_origin !== filters.productOrigin) {
                return false;
            }
            ...
            if (filters.stockStatus === "quarantine" && row.quarantine <= 0) return false;
